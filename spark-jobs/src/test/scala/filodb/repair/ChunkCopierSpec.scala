/* FIXME: Commented out for now because it interferes with the downsampler tests due to global config.
...
class ChunkCopierSpec extends FunSpec with Matchers with BeforeAndAfterAll with ScalaFutures {
  ...
  // Setting a global system property like this is a bad idea, but it's necessary because the
  // downsampler and it's tests only work with global state. The chunk copier and tests are less
  // vulnerable to global state, and so if the downsampler is fixed then this line can go away.
