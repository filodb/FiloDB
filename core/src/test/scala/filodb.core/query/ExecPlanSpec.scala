...
class ExecPlanSpec extends FunSpec with Matchers with BeforeAndAfter with ScalaFutures {
  ...
  val partKeys = (0 to 9).map(n => partKeyBuilder.addFromObjects(s"Series $n"))
  ...
  it("LocalVectorReader should return result with vectors and proper schema when execute") {
    ...
    plan.executeToResult(memStore, dataset1).runAsync.futureValue match {
      case r @ VectorListResult(range, schema, vectorList) =>
        ...
        vectorList.zip(partKeys).foreach { case (vect, k) => dataset1.partKeySchema.equals(vect.info.get.offset, k) }
        ...
        r.toRowReaders.toSeq.map(_._1.get.offset).zip(partKeys).foreach(dataset1.partKeySchema.equals)
  ...
  //   val stream = Observable.fromIterable(records(dataset1, linearMultiSeries().take(20)).grouped(5).toSeq)
  ...
  //   val keys = (2 to 4).map(n => partKeyBuilder.addFromObjects(s"Series $n"))
  //                      .map(dataset1.partKeySchema.toBytes)
