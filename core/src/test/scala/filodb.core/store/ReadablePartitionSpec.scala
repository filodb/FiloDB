...
class ReadablePartitionSpec extends AnyFunSpec with Matchers with BeforeAndAfterAll {
  ...
  it("areCompatibleMetricTypes should return true for delta and otel-delta") {
    ...
    Utils.areCompatibleMetricTypes(p1.schema.schemaHash, p2.schema.schemaHash) shouldEqual true
  }

  it("areCompatibleMetricTypes should return true for otel-cumul and otel-delta") {
    val rawData = RawPartData(Array.empty, Seq.empty)
  ...
  it("areCompatibleMetricTypes should return true for otel-delta and delta") {
    val rawData = RawPartData(Array.empty, Seq.empty)
  ...
  it("areCompatibleMetricTypes should return true for cumulative and otel-cumulative") {
    val rawData = RawPartData(Array.empty, Seq.empty)
  ...
  it("areCompatibleMetricTypes should return true for preagg-delta and preagg-otel-delta") {
    val rawData = RawPartData(Array.empty, Seq.empty)
  ...
  it("areCompatibleMetricTypes should return true for preagg-otel-delta and preagg-delta") {
    val rawData = RawPartData(Array.empty, Seq.empty)
  ...
  it("areCompatibleMetricTypes should return true for promCounter and deltaCounter") {
    val rawData = RawPartData(Array.empty, Seq.empty)
    val p1 = new PagedReadablePartition(Schemas.promCounter, 0, -1, rawData, 10)
    val p2 = new PagedReadablePartition(Schemas.deltaCounter, 0, -1, rawData, 10)
    Utils.areCompatibleMetricTypes(p1.schema.schemaHash, p2.schema.schemaHash) shouldEqual true
  }

  it("areCompatibleMetricTypes should return true for otel-cumulative and prom histogram") {
    val rawData = RawPartData(Array.empty, Seq.empty)
    val p1 = new PagedReadablePartition(Schemas.otelCumulativeHistogram, 0, -1, rawData, 10)
  ...
  it("areCompatibleMetricTypes should return true if schema matches paged readable partition") {
    val rawData = RawPartData(Array.empty, Seq.empty)
    ...
    Utils.areCompatibleMetricTypes(p3.schema.schemaHash, p4.schema.schemaHash) shouldEqual true
    ...
    Utils.areCompatibleMetricTypes(p5.schema.schemaHash, p6.schema.schemaHash) shouldEqual true
    ...
    Utils.areCompatibleMetricTypes(p7.schema.schemaHash, p8.schema.schemaHash) shouldEqual true
    ...
    Utils.areCompatibleMetricTypes(p9.schema.schemaHash, p10.schema.schemaHash) shouldEqual true
    ...
    Utils.areCompatibleMetricTypes(p11.schema.schemaHash, p12.schema.schemaHash) shouldEqual true
  }

  it("areCompatibleMetricTypes should return true if schema matches timeseries partition") {
  ...
  it("areCompatibleMetricTypes should return false if schema does not matches timeseries partition") {
    val storeConfig = StoreConfig(GlobalConfig.defaultFiloConfig.getConfig("downsampler.downsample-store-config"))
    val offheapMem = new OffHeapMemory(Seq(Schemas.gauge, Schemas.promCounter, Schemas.promHistogram, Schemas.untyped),
      Map.empty, 100, storeConfig)
    val shardInfo = TimeSeriesShardInfo(
    ...
    val p1 = new TimeSeriesPartition(0, Schemas.gauge, 0, shardInfo, 1)
    val p2 = new TimeSeriesPartition(0, Schemas.promHistogram, 0, shardInfo, 1)
    Utils.areCompatibleMetricTypes(p1.schema.schemaHash, p2.schema.schemaHash) shouldEqual false
    ...
    Utils.areCompatibleMetricTypes(p5.schema.schemaHash, p6.schema.schemaHash) shouldEqual false

    val p7 = new TimeSeriesPartition(0, Schemas.gauge, 0, shardInfo, 1)
    val p8 = new TimeSeriesPartition(0, Schemas.otelDeltaHistogram, 0, shardInfo, 1)
    Utils.areCompatibleMetricTypes(p7.schema.schemaHash, p8.schema.schemaHash) shouldEqual false
  }
}
