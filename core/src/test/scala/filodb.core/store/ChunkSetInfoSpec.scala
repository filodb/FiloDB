...
class ChunkSetInfoSpec extends NativeVectorTest {
  ...
  val info1 = ChunkSetInfo(memFactory, dataset.schema.data, chunkID(firstKey, 0), 0, 5000, lastKey)
  ...
  it("should find intersection range when one of timestamps match") {
    ...
    val info1 = ChunkSetInfo(memFactory, dataset.schema.data, chunkID(ts - 10000, 0), 0, 1, ts + 30000)
  ...
  it("should not find intersection if key1 is greater than key2") {
    val info1 = ChunkSetInfo(memFactory, dataset.schema.data, chunkID(1000, 0), 0, 1, 2000)
