...
// 0: Timestamp, 1: Histogram, 2: Max/Double
final class TransientHistMinMaxRow(var max: Double = 0.0, var min: Double = 0.0) extends TransientHistRow() {
  override def setDouble(columnNo: Int, valu: Double): Unit =
  ...
  override def toString: String = s"TransientHistMinMaxRow(t=$timestamp, h=$value, max=$max, min=$min)"
}
