      ...
      finalRes._1
        .map {
          case srv: SerializableRangeVector =>
            ...
            if (failOnLimit && numResultSamples > limit)
          ...
          case rv: RangeVector =>
            ...
            val srv = SerializableRangeVector(rv, builder, recSchema, numResultSamples, limit, failOnLimit)
            numResultSamples += srv.numRows
            srv
        }
        .toListL
