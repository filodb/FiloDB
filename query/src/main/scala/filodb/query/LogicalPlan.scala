...
object LogicalPlan {
  ...
  def findLeafLogicalPlans (logicalPlan: LogicalPlan) : Seq[LogicalPlan] = {
   logicalPlan match {
     ...
     case lp: ScalarBinaryOperation       => val lhsLeafs =
                                              if (lp.lhs.isRight) findLeafLogicalPlans(lp.lhs.toOption.get) else Nil
                                             val rhsLeafs =
                                              if (lp.rhs.isRight) findLeafLogicalPlans(lp.rhs.toOption.get) else Nil
                                             lhsLeafs ++ rhsLeafs
