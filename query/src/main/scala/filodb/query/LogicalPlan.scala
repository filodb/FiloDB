...
/**
 *  Subquery represents a series of N points generated by running N homogeneous queries
 *  where N = ((startMs - endMs) / stepMs) + 1
 *  The subqueries are represtened by a PeriodicSeriesPlan and conceptually
 *  supposed to be run N times with its own start time ranging from StartMs to endMs
 *  (including endMs time).
 *  Children of subquery are supposed to return an instant vector and the results
 *  logically need to be stitched to form a range vector.
 *  ExecutionPlan generated out of Subquery logical plan can be optimized in such a way
 *  that we do NOT issue N separate subqueries but instead issue one large query and simulate
 *  subquery functionality similarly to execution plans of PeriodicSeriesWithWindowing.
 *  It is important to understand, however, that issuing one large time series query
 *  is not always the most optimal solution, in these cases query planner should
 *  indeed generate execution plan with N separate subqueries.
 *  Finally, currently we "materialize" subqueries literally creating a logical plan
 *  for each of them. Most likely, we should be able to do better later on in Phase III
 *
 * @param subqueries - the query that conceptually should be repeated N times
 * @param startMs
 * @param stepMs
 * @param endMs
 */
case class Subquery(
  ...
  override def children: Seq[LogicalPlan] = subqueries
...
case class RangeFunctionPlan(
  ...
  override def children: Seq[LogicalPlan] = Seq(nestedPlan)
  override def replacePeriodicSeriesFilters(filters: Seq[ColumnFilter]): PeriodicSeriesPlan = ???
}
