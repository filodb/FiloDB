...
object Ptr {
  ...
  final case class U8(addr: Long) extends AnyVal {
    ...
    final def getU8(base: Any): Int = UnsafeUtils.getByte(base, addr) & 0x00ff
  ...
  final case class MutU8(addr: Long) extends AnyVal {
    final def set(base: Any, num: Int): Unit = UnsafeUtils.setByte(base, addr, num.toByte)
  }
  ...
  final case class U16(addr: Long) extends AnyVal {
    ...
    final def getU16(base: Any): Int = UnsafeUtils.getShort(base, addr) & 0x00ffff

    final def asMut: MutU16 = MutU16(addr)
  }

  final case class MutU16(addr: Long) extends AnyVal {
    final def set(base: Any, num: Int): Unit = UnsafeUtils.setShort(base, addr, num.toShort)
  }
  ...
  final case class I32(addr: Long) extends AnyVal {
    ...
    final def getI32(base: Any): Int = UnsafeUtils.getInt(base, addr)

    final def asMut: MutI32 = MutI32(addr)
  }

  final case class MutI32(addr: Long) extends AnyVal {
    final def set(base: Any, num: Int): Unit = UnsafeUtils.setInt(base, addr, num)
  }
}