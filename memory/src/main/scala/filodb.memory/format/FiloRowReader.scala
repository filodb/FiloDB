...
abstract class MutableFiloRowReader extends RowReader {
  ...
  final def getInt(columnNo: Int): Int =
    reader(columnNo).asIntReader.apply(vectAccessor(columnNo), vectAddr(columnNo), rowNo)
  final def getLong(columnNo: Int): Long =
    reader(columnNo).asLongReader.apply(vectAccessor(columnNo), vectAddr(columnNo), rowNo)
  final def getDouble(columnNo: Int): Double =
    reader(columnNo).asDoubleReader(vectAccessor(columnNo), vectAddr(columnNo), rowNo)
  ...
  override final def filoUTF8String(columnNo: Int): ZeroCopyUTF8String =
    reader(columnNo).asUTF8Reader.apply(vectAccessor(columnNo), vectAddr(columnNo), rowNo)
