...
abstract class MutableFiloRowReader extends RowReader {
  ...
  final def getInt(columnNo: Int): Int = reader(columnNo).asIntReader.apply(vectAddr(columnNo), rowNo)
  final def getLong(columnNo: Int): Long = reader(columnNo).asLongReader.apply(vectAddr(columnNo), rowNo)
  final def getDouble(columnNo: Int): Double = reader(columnNo).asDoubleReader(vectAddr(columnNo), rowNo)
