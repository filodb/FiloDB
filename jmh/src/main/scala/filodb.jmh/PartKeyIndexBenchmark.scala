...
abstract class PartKeyIndexBpackage filodb.jmh
...
abstract class PartKeyIndexBenchmark {
  ...
  val numSeries = 14000 // TEST-ONLY local change (was 1000000) to match the 14k pod test; do not commit
  ...
  // ===== TEST-ONLY (do not commit): replay real diagnostic EXPENSIVE pod regexes on `instance` =====
  // Patterns come from the diagnostic's extracted query set (/tmp/t14k/queries.tsv).
  private val diagExpPats: Array[String] = {
    val fallback = Array(".*prod-p148-.*")
    try {
      val rows = scala.io.Source.fromFile("/tmp/t14k/queries.tsv").getLines().drop(1)
        .map(_.split("\t", -1)).filter(_.length > 4).toArray
      val pats = rows.filter(r => r(1) == "expensive" && r(3) == "=~").map(r => r(4))
      if (pats.nonEmpty) pats else fallback
    } catch { case _: Throwable => fallback }
  }
  println(s"diag expensive patterns loaded: ${diagExpPats.length}")
  private var diagExpCur = 0
  ...
  def partIdsLookupWithSuffixRegexFilters(): Unit = {
    // TEST-ONLY: replay real diagnostic EXPENSIVE (unanchored) pod regexes on the `instance` field,
    // all scoped to the populated _ns_=App-0 (no empty-namespace lookups). 8 real lookups per op.
    cforRange ( 0 until 8 ) { i =>
      val pat = diagExpPats(diagExpCur % diagExpPats.length)
      diagExpCur += 1
      partKeyIndex.partIdsFromFilters(
        Seq(ColumnFilter("_ns_", Filter.Equals("App-0")),
          ColumnFilter("_ws_", Filter.Equals("demo")),
          ColumnFilter("_metric_", Filter.Equals("heap_usage0")),
          ColumnFilter("instance", Filter.EqualsRegex(pat))),
        now,
        currentLookupTime())
    }
  }
