...
class HighAvailabilityPlannerSpec extends AnyFunSpec with Matchers with PlanValidationSpec {
  ...
  */
  ...
  it("should generate PromQLFlightRemoteExec when failure is present only in local and remote flight-gRPC is configured") {
    val to = 10000
    val from = 100
    val intervalSelector = IntervalSelector(from, to)
    val raw = RawSeries(rangeSelector = intervalSelector, filters = f1, columns = Seq("value"))
    ...
    execPlan.asInstanceOf[PromQLFlightRemoteExec].queryEndpoint shouldEqual "grpcEndpoint"
  ...
  it("should generate MetadataRemoteExec for TsCardinalities when local failure is present") {
    ...
    val expected = """E~MetadataRemoteExec(PromQlQueryParams(sum(heap_usage0),100,1,1000,None,true), PlannerParams(filodb,None,None,None,None,60000,PerQueryLimits(1000000,1000000,18000000,100000,100000,300000000,1000000,200000000),PerQueryLimits(50000,1000000,15000000,50000,50000,150000000,500000,100000000),None,None,None,false,86400000,86400000,false,false,false,false,true,10,false,true,TreeSet(),LegacyFailoverMode,None,None,None,None,SamplesScannedConfig(true,false,false,false,false,false,None,1.0,25.0,50.0,1.0,0.0,0.0,Map(),Map(),Map(),0.0,0.0,0.0,Map(),Map(),Map())), queryEndpoint=localhost, requestTimeoutMs=10000) on InProcessPlanDispatcher(QueryConfig(10 seconds,300000,1,50,antlr,true,true,Some(p1),Some(10000),Some(localhost),None,25,true,false,true,Set(),Set(),Some(plannerSelector),Map(filodb-query-exec-metadataexec -> 65536, filodb-query-exec-aggregate-large-container -> 65536),RoutingConfig(false,1800000 milliseconds,true,0,Set(),_ws_),CachingConfig(true,2048),false))"""
