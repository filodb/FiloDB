...
class ShardKeyRegexPlannerSpec extends AnyFunSpec with Matchers with ScalaFutures with PlanValidationSpec {
  ...
  it("should generate Exec plan for simple query") {
    ...
    execPlan.children(1).asInstanceOf[MultiSchemaPartitionsExec].filters.
      contains(ColumnFilter("_ns_", EqualsRegex("App.*"))) shouldEqual(true)
  ...
  it("should generate Exec plan for subquery with windowing") {
    ...
    val engine = new ShardKeyRegexPlanner(dataset, mpp, shardKeyMatcherFn, mppPartitionLocationProvider, queryConfig)
  ...
  it("should generate Exec plan for Scalar Binary Operation") {
    ...
    execPlan.children(1).queryContext.origQueryParams.asInstanceOf[PromQlQueryParams].promQl shouldEqual
      "test{_ws_=\"demo\",_ns_=~\"App.*\",instance=\"Inst-1\"}"
    execPlan.children(0).queryContext.origQueryParams.asInstanceOf[PromQlQueryParams].promQl shouldEqual
      "test{_ws_=\"demo\",_ns_=~\"App.*\",instance=\"Inst-1\"}"
    execPlan.children(0).rangeVectorTransformers(0).isInstanceOf[PeriodicSamplesMapper] shouldEqual true
    execPlan.children(1).asInstanceOf[MultiSchemaPartitionsExec].filters.
      contains(ColumnFilter("_ns_", EqualsRegex("App.*"))) shouldEqual(true)
    execPlan.children(0).asInstanceOf[MultiSchemaPartitionsExec].filters.
      contains(ColumnFilter("_ns_", EqualsRegex("App.*"))) shouldEqual(true)
  ...
  it("should generate Exec plan for histogram quantile for Aggregate query") {
    ...
    execPlan.children(0).rangeVectorTransformers.length shouldEqual 2
    execPlan.children(0).rangeVectorTransformers(0).isInstanceOf[PeriodicSamplesMapper] shouldEqual true
    execPlan.children(0).rangeVectorTransformers(1).isInstanceOf[AggregateMapReduce] shouldEqual true
    execPlan.children(1).asInstanceOf[MultiSchemaPartitionsExec].filters.
      contains(ColumnFilter("_ns_", EqualsRegex("App.*"))) shouldEqual(true)
    execPlan.children(0).asInstanceOf[MultiSchemaPartitionsExec].filters.
      contains(ColumnFilter("_ns_", EqualsRegex("App.*"))) shouldEqual(true)
  ...
  it("should generate Exec plan for scalar - time()") {
    ...
    execPlan.rangeVectorTransformers.head.asInstanceOf[ScalarOperationMapper].funcParams.head.
      isInstanceOf[ExecPlanFuncArgs] shouldEqual true
    execPlan.rangeVectorTransformers.head.asInstanceOf[ScalarOperationMapper].funcParams.head.
      asInstanceOf[ExecPlanFuncArgs].execPlan.isInstanceOf[LocalPartitionDistConcatExec] shouldEqual true
    ...
    multiPartitionExec.children(1).queryContext.origQueryParams
      .asInstanceOf[PromQlQueryParams].promQl shouldEqual """test{_ws_="demo",_ns_=~"App.*"}"""
    multiPartitionExec.children(0).queryContext.origQueryParams
      .asInstanceOf[PromQlQueryParams].promQl shouldEqual """test{_ws_="demo",_ns_=~"App.*"}"""
  }
  ...
  it("should generate Exec plan for Binary join with regex") {
    ...
    lhs.children.length shouldEqual 4
    lhs.children(0).isInstanceOf[MultiSchemaPartitionsExec] shouldEqual true
    lhs.children(1).asInstanceOf[MultiSchemaPartitionsExec].filters.
      contains(ColumnFilter("_ns_", EqualsRegex("App.*"))) shouldEqual(true)
    lhs.children(0).asInstanceOf[MultiSchemaPartitionsExec].filters.
      contains(ColumnFilter("_ns_", EqualsRegex("App.*"))) shouldEqual(true)
    val rhs = execPlan.children(1).asInstanceOf[LocalPartitionDistConcatExec]
    rhs.children(0).isInstanceOf[MultiSchemaPartitionsExec] shouldEqual true
    rhs.children(1).asInstanceOf[MultiSchemaPartitionsExec].filters.
      contains(ColumnFilter("_ns_", EqualsRegex("App.*"))) shouldEqual(true)
    rhs.children(0).asInstanceOf[MultiSchemaPartitionsExec].filters.
      contains(ColumnFilter("_ns_", EqualsRegex("App.*"))) shouldEqual(true)
  }
  ...
  it("should handle push down aggregation to right level case 1") {
    ...
    mpExec.isInstanceOf[LocalPartitionReduceAggregateExec] shouldEqual true
    mpExec.asInstanceOf[LocalPartitionReduceAggregateExec].aggrOp shouldEqual Count
    ...
    mpExec.children match {
      case children: Seq[ExecPlan] if children.size == 4 =>
        children.map(_.queryContext.origQueryParams.asInstanceOf[PromQlQueryParams].promQl).toSet shouldEqual Set("""count(test1{_ws_="demo",_ns_=~"App-.*"}) by (foo)""")
        children.head.isInstanceOf[MultiSchemaPartitionsExec] shouldEqual true
      case _ => fail("Expected four children")
    }
  }
  ...
  it("should handle push down aggregation to right level case 2") {
    ...
    execPlan.queryContext.origQueryParams.asInstanceOf[PromQlQueryParams].promQl shouldEqual
      """sum(count(test1{_ws_="demo",_ns_=~"App-.*"}) by (foo))"""
  }
  ...
  it("should handle multi partition multi level binary join appropriately with instant fn") {
    ...
    execPlan.children match {
      case children: Seq[ExecPlan] if children.size == 4 =>
        children.map(_.queryContext.origQueryParams.asInstanceOf[PromQlQueryParams].promQl).toSet shouldEqual Set("""test1{_ws_="demo",_ns_=~"App-1"}""")
        children.head.isInstanceOf[MultiSchemaPartitionsExec] shouldEqual true
      case _ => fail("Expected four children")
    }
  ...
  it("should handle simple instant function applied to single partition raw series with regex") {
    ...
    execPlan.children match {
      case children: Seq[ExecPlan] if children.size == 4 =>
        children.map(_.queryContext.origQueryParams.asInstanceOf[PromQlQueryParams].promQl).toSet shouldEqual Set("""test1{_ws_="demo",_ns_=~"App-*"}""")
        children.head.isInstanceOf[MultiSchemaPartitionsExec] shouldEqual true
      case _ => fail("Expected four children")
    }
  }
  ...
  it("should generate the appropriate plan with label_join") {
    ...
    val execPlan = engine.materialize(lp, QueryContext(origQueryParams = promQlQueryParams))
    execPlan.isInstanceOf[LocalPartitionDistConcatExec] shouldEqual (true)
    execPlan.children.size shouldEqual 4
    ...
    execPlan.children.foreach( x => {
      x.isInstanceOf[MultiSchemaPartitionsExec] shouldEqual true
      x.dispatcher.isInstanceOf[ActorPlanDispatcher] shouldEqual (true)
    })
    execPlan.children match {
      case children: Seq[ExecPlan] if children.size == 4 =>
        children.map(_.queryContext.origQueryParams.asInstanceOf[PromQlQueryParams].promQl).toSet shouldEqual Set("""foo{_ws_="Demo",_ns_=~"App*"}""")
        children.head.isInstanceOf[MultiSchemaPartitionsExec] shouldEqual true
      case _ => fail("Expected four children")
    }
  }
  ...
  it("should push down absent function appropriately to individual partitions") {
    ...
    execPlan.children.foreach( x => {
      ...
      x.dispatcher.isInstanceOf[ActorPlanDispatcher] shouldEqual (true)
      x.rangeVectorTransformers.isEmpty shouldEqual false
      x.asInstanceOf[ReduceAggregateExec].aggrOp shouldEqual Sum

    })
    execPlan.children.head.children match {
      case children: Seq[ExecPlan] if children.size == 4 =>
        children.map(_.queryContext.origQueryParams.asInstanceOf[PromQlQueryParams].promQl).toSet shouldEqual Set("""sum(absent(foo{_ws_="Demo",_ns_=~"App*"}))""")
        children.head.isInstanceOf[MultiSchemaPartitionsExec] shouldEqual true
      case _ => fail("Expected four children")
    }
  }
  ...
  it("should execute absent in-process") {
    ...
    child.isInstanceOf[LocalPartitionDistConcatExec] shouldEqual true
  ...
  it("should materialize vector appropriately for multi partition") {
    ...
    execPlan.children match {
      case children: Seq[ExecPlan] if children.size == 4 =>
        children.map(_.queryContext.origQueryParams.asInstanceOf[PromQlQueryParams].promQl).toSet shouldEqual Set("""sum(foo{_ws_="demo",_ns_=~"App*"})""")
        children.head.isInstanceOf[MultiSchemaPartitionsExec] shouldEqual true
      case _ => fail("Expected four children")
    }
  }
