...
class PlannerUtilSpec  extends AnyFunSpec with Matchers {
  ...
  it ("should rewrite the plan to make RawSeries support remote calls") {
    ...
    assert(LogicalPlan.findLeafLogicalPlans(lp1) forall{
      case rs: RawSeries  =>
        rs.supportsRemoteDataCall &&  rs.lookbackMs.isDefined && rs.lookbackMs.get == 600000L && !rs.offsetMs.isDefined
      case _              => false
    })
    ...
    val lp6 = rewritePlanWithRemoteRawExport(
      Parser.queryRangeToLogicalPlan(q6, TimeStepParams(start, 0, end)), Some(600000))
    ...
    val lp7 = rewritePlanWithRemoteRawExport(Parser.queryRangeToLogicalPlan(q7, TimeStepParams(start, 0, end)),
      Some(600000))
