...
private[filodb] final class NodeCoordinatorActor(metaStore: MetaStore,
  ...
  def coordinatorReceive: Receive = LoggingReceive {
    ...
    case e: ShardForwardAction        => forward(e, sender())
  ...
  /** Forwards shard actions/commands to the ingester for the given dataset.
    * TODO version match if needed, when > 1, currently only 0.
    */
  private def forward(action: ShardForwardAction, origin: ActorRef): Unit =
    ingesters.get(action.ref) match {
      case Some(actor) =>
        actor.tell(action, origin)
      case _ =>
        logger.warn(s"No IngestionActor for dataset ${action.ref}")
    }
