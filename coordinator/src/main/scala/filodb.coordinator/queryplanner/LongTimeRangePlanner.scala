    ...
    logicalPlan match {
      case p: PeriodicSeriesPlan =>
        ...
        if (!logicalPlan.isRoutable)
          ...
          logger.info("materializing against ds cluster query={} logicalPlan={}", qContext.origQueryParams, logicalPlan)
          ...
          logger.info("materializing against ds cluster query={} logicalPlan={}", qContext.origQueryParams, downsampleLp)
