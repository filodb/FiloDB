...
class ShardKeyRegexPlanner(val dataset: Dataset,
  ...
  override def walkLogicalPlanTree(logicalPlan: LogicalPlan,
    ...
    if (hasSameShardKeyFilters && assignments.size == 1) {
  ...
  private def generateExecWithoutRegex(logicalPlan: LogicalPlan, nonMetricShardKeyFilters: Seq[ColumnFilter],
                                       qContext: QueryContext, pushdownPlan: Boolean = false): Seq[ExecPlan] = {
    val shardKeyMatches = shardKeyMatcher(nonMetricShardKeyFilters)
    generateExec(logicalPlan, shardKeyMatches, qContext, pushdownPlan)
  }
  ...
  private def generateExecForEachAssignment(logicalPlan: LogicalPlan,
    ...
    val nonSplitPlans = assignmentToKeyGroups.flatMap{ case (partition, keyGroups) =>
