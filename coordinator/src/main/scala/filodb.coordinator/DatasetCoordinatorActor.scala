...
private[filodb] class DatasetCoordinatorActor(projection: RichProjection,
  ...
  private def writeToMemTable(): Unit = if (tempRows.nonEmpty) {
    ...
    if (kamonEnabled) {
      kamonRowsIngested.foreach(_.increment(tempRows.length))
    }
  ...
  private def canIngest: Boolean = {
    ...
    if (activeRows >= maxRowsPerTable) {
      ...
      if (kamonEnabled) {
        kamonMemtableFull.foreach(_.increment)
      }
      false
    } else {
      ...
      if (freeMB < minFreeMb) {
        ...
        if (kamonEnabled) {
          kamonOOMCount.foreach(_.increment)
        }
        sys.runtime.gc()
        false
      }
  ...
  private def handleFlushDone(): Unit = {
    ...
    flushesSucceeded += 1
    if (kamonEnabled) {
      kamonFlushesDone.foreach(_.increment)
    }
  ...
  private def handleFlushErr(t: Throwable): Unit = {
    ...
    flushesFailed += 1
    if (kamonEnabled) {
      kamonFlushesFailed.foreach(_.increment)
    }
