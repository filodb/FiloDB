...
/*
 ...
 antlr -package filodb.prometheus.antlr -visitor -no-listener PromQL.g4
...
literal: NUMBER | STRING | INF | NAN;

// Number format is non-standard.
// It doesn't support hex and it doesn't require a digit after the decimal point.
NUMBER: (
      [0-9]* '.'? [0-9]+ ([eE][-+]?[0-9]+)?
    | [0-9]+ '.'
    | '0' [xX] [0-9a-fA-F]+
