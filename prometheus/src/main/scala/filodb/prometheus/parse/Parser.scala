...
object Parser extends StrictLogging {
  ...
  def parseQueryWithoutNameValidation(query: String): Expression = {
    mode match {
  ...
  def parseQuery(query: String): Expression = {
    parseQueryWithoutNameValidation(query).validateNames()
  }
  ...
  private def parseQueryWithPrecedence(query: String, modeParam: Mode = mode): Expression = {
    modeParam match {
      case Antlr => AntlrParser.parseQuery(query).validateNames()
      case Legacy => LegacyParser.parseQueryWithPrecedence(query).validateNames()
      case Shadow => {
        val expr = LegacyParser.parseQueryWithPrecedence(query).validateNames()
        try {
          val antlrExpr = AntlrParser.parseQuery(query).validateNames()
  ...
  def parseFilter(query: String): InstantExpression = {
    val expr = parseQuery(query).validateNames()
  ...
  def metadataQueryToNameUnvalidatedPlan(query: String, timeParams: TimeRangeParams,
                                 fetchFirstLastSampleTimes: Boolean = false): LogicalPlan = {
    val expression = parseQueryWithoutNameValidation(query)
    expression match {
      case p: InstantExpression => p.toMetadataPlan(timeParams, fetchFirstLastSampleTimes)
      case _ => throw new UnsupportedOperationException()
    }
